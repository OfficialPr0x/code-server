...
exports.setDefaultLogLevel = exports.toCodeArgs = exports.shouldOpenInExistingInstance = exports.redactArgs = exports.parse = exports.optionDescriptions = exports.options = exports.OptionalString = exports.LogLevel = exports.Optional = exports.AuthType = exports.Feature = void 0;
...
const parse = (argv, opts) => {
    ...
    logger_2.logger.debug(`parsed ${source}`, (0, logger_1.field)("args", (0, exports.redactArgs)(args)));
    return args;
};
exports.parse = parse;
/**
...
async function setDefaults(cliArgs, configArgs) {
    ...
    (0, exports.setDefaultLogLevel)(args.log);
    ...
    if (args.cert && !args.cert.value) {
        const { cert, certKey } = await (0, util_1.generateCertificate)(args["cert-host"] || "localhost");
        args.cert = {
            value: cert,
        };
...
async function readConfigFile(configPath) {
    ...
    try {
        const generatedPassword = await (0, util_1.generatePassword)();
        await fs_1.promises.writeFile(configPath, defaultConfigFile(generatedPassword), {
            flag: "wx", // wx means to fail if the path exists.
        });
        logger_2.logger.info(`Wrote default config file to ${configPath}`);
    }
    catch (error) {
 ...
 */
const shouldOpenInExistingInstance = async (args, sessionSocket) => {
    // Always use the existing instance if we're running from VS Code's terminal.
    if (process.env.VSCODE_IPC_HOOK_CLI) {
        logger_2.logger.debug("Found VSCODE_IPC_HOOK_CLI");
    ...
    if (openInFlagCount > 0) {
        logger_2.logger.debug("Found --reuse-window or --new-window");
    ...
    if (Object.keys(args).length === 1 && typeof args._ !== "undefined" && args._.length > 0) {
        ...
        const socketPath = await client.getConnectedSocketPath(paths[0]);
        if (socketPath) {
            logger_2.logger.debug("Found existing code-server socket");
            return socketPath;
        }
    }
    return undefined;
...
const setDefaultLogLevel = (logLevel) => {
    // Set @coder/logger level
    switch (logLevel) {
        case LogLevel.Trace:
            logger_2.coderLogger.level = logger_1.Level.Trace;
            break;
        case LogLevel.Debug:
            logger_2.coderLogger.level = logger_1.Level.Debug;
            break;
        case LogLevel.Info:
            logger_2.coderLogger.level = logger_1.Level.Info;
            break;
        case LogLevel.Warn:
            logger_2.coderLogger.level = logger_1.Level.Warn;
            break;
        case LogLevel.Error:
            logger_2.coderLogger.level = logger_1.Level.Error;
            break;
    }
};
